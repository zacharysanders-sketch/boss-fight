class Player {
  ...
  draw(ctx) {
    ...
    const sx = this.isAttacking ? this.x + 85 : this.x + 52;
    ctx.beginPath();
  ...
  attack(boss) {
    const dmg = 24 + Math.floor(Math.random() * 14);
  ...
  specialAttack(boss) {
    ...
    const dmg = 58 + Math.floor(Math.random() * 22);
    boss.health = Math.max(0, boss.health - dmg);
